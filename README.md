...
The primary navigation is consolidated onto homepage anchors:

- `Portfolio` at `/#portfolio`
- `CV` at `/#cv`
- `Drive` at `/#drive`

Every public project has a direct route at `/projects/[slug]`. Legacy section routes redirect to the matching homepage anchor. Flipper Zero exploration and the coming-soon Cutlery Sorting Machine are intentionally public. The unfinished standalone Investment section remains excluded from navigation and search indexing. Contact details live in the one-line footer.
...
The Sundvolden Hotel project is intentionally draft-only until Gard supplies the approved public narrative: goal, context, Gard's role, work performed, outcome, constraints, and any approved media.
...
Keep videos close to or below 4 MB, provide a poster, and avoid offscreen preloading. Never put a private CV, restricted portfolio, or confidential report in `public/`; every file in that directory is downloadable. The approved public CV is `public/Gard-Laeskogen-CV.pdf`.
...
Create `.env.local` only for optional public integrations:
...
It runs content validation, lint, TypeScript checking, and a production build. Useful individual commands are:
...
For a direct Cloudflare Worker deployment, configure any optional public environment variables and run:

```bash
pnpm deploy
```

That command builds the vinext application and deploys the generated `dist/server/wrangler.json` configuration. The root `wrangler.jsonc` supplies the Worker entry point and non-secret defaults.
